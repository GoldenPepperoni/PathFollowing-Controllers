...
envs = gymnasium.make("PyFlyt/Fixedwing-CCDubinsPath-v0", render_mode=None, angle_representation="euler", flight_dome_size=200, turning_radius=50, num_targets=len(custom_yaw_targets), custom_targets=custom_targets, custom_yaw_targets=custom_yaw_targets)
...
if makeGif:
    ...
    imgs[0].save("CC_spiral.gif", save_all=True, append_images=imgs[1:], duration=100/3, loop=0)

if makePlots:
    plotXY(desiredPath, actualPath, "Horizontal trajectory (CC_spiral)")
    plotZ(desiredPath, actualPath, "Vertical trajectory (CC_spiral)")
    plot3D(desiredPath, [actualPath], "3D trajectory (CC_spiral)")
    plotCtrlTraces(ctrlTraces, tArray, "Control traces (CC_spiral)")
    plt.show()
